...
**Dsicussion**:
- Why does a linear model only represent the top features? Linear neural networks (NNs without activations) can be thought of optimizing a closed-form solution: 
\[
L ~ \sum_i I_i(1 - ||W_i||^2)^2 + \sum_{i \neq j} (W_j \cdot W_i + b_j)^2
\]
